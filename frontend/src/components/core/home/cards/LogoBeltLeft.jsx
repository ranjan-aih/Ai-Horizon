const LogoBeltLeft = ({ direction, logos = [] }) => {
  return (
    <div className='w-full overflow-hidden py-8'>
      <style>
        {`
          @keyframes scrollLeft {
            0% { transform: translateX(0); }
            100% { transform: translateX(-50%); }
          }
          @keyframes scrollRight {
            0% { transform: translateX(-50%); }
            100% { transform: translateX(0); }
          }
          .scroll-animation-left {
            display: flex;
            gap: 16px;
            width: max-content;
            animation: ${
              direction === 'left' ? 'scrollLeft' : 'scrollRight'
            } 50s linear infinite;
          }
          .logo-item {
            transition: all 0.3s ease;
          }
          .logo-item .white-svg {
            display: block;
          }
          .logo-item .colored-svg {
            display: none;
          }
          .logo-item:hover .white-svg {
            display: none;
          }
          .logo-item:hover .colored-svg {
            display: block;
          }
          .logo-item:hover .logo-text {
            color: #ffffff;
          }
        `}
      </style>
      <div className='scroll-animation-left'>
        {[...logos, ...logos, ...logos].map((logo, index) => (
          <div
            key={index}
            className='flex items-center gap-3 whitespace-nowrap flex-shrink-0 logo-item cursor-pointer'
          >
            {logo.type === 'google' ? (
              <>
                {/* White Google SVG */}
                <svg
                  className='white-svg'
                  xmlns='http://www.w3.org/2000/svg'
                  x='0px'
                  y='0px'
                  width='48'
                  height='48'
                  viewBox='0 0 48 48'
                >
                  <path
                    fill='#ffffff'
                    d='M38.193,18.359c-0.771-2.753-2.319-5.177-4.397-7.03l-4.598,4.598	c1.677,1.365,2.808,3.374,3.014,5.648v1.508c0.026,0,0.05-0.008,0.076-0.008c2.322,0,4.212,1.89,4.212,4.212S34.61,31.5,32.288,31.5	c-0.026,0-0.05-0.007-0.076-0.008V31.5h-6.666H24V38h8.212v-0.004c0.026,0,0.05,0.004,0.076,0.004C38.195,38,43,33.194,43,27.288	C43,23.563,41.086,20.279,38.193,18.359z'
                  ></path>
                  <path
                    fill='#ffffff'
                    d='M19.56,25.59l4.72-4.72c-0.004-0.005-0.008-0.009-0.011-0.013l-4.717,4.717	C19.554,25.579,19.557,25.584,19.56,25.59z'
                    opacity='.5'
                  ></path>
                  <path
                    fill='#ffffff'
                    d='M19.56,25.59l4.72-4.72c-0.004-0.005-0.008-0.009-0.011-0.013l-4.717,4.717	C19.554,25.579,19.557,25.584,19.56,25.59z'
                    opacity='.5'
                  ></path>
                  <path
                    fill='#ffffff'
                    d='M24,7.576c-8.133,0-14.75,6.617-14.75,14.75c0,0.233,0.024,0.46,0.035,0.69h6.5	c-0.019-0.228-0.035-0.457-0.035-0.69c0-4.549,3.701-8.25,8.25-8.25c1.969,0,3.778,0.696,5.198,1.851l4.598-4.598	C31.188,9.003,27.761,7.576,24,7.576z'
                  ></path>
                  <path
                    fill='#ffffff'
                    d='M15.712,31.5L15.712,31.5c-0.001,0-0.001,0-0.002,0c-0.611,0-1.188-0.137-1.712-0.373	l-4.71,4.71C11.081,37.188,13.301,38,15.71,38c0.001,0,0.001,0,0.002,0v0H24v-6.5H15.712z'
                    opacity='.5'
                  ></path>
                  <path
                    fill='#ffffff'
                    d='M15.712,31.5L15.712,31.5c-0.001,0-0.001,0-0.002,0c-0.611,0-1.188-0.137-1.712-0.373l-4.71,4.71	C11.081,37.188,13.301,38,15.71,38c0.001,0,0.001,0,0.002,0v0H24v-6.5H15.712z'
                  ></path>
                  <path
                    fill='#ffffff'
                    d='M11.5,27.29c0-2.32,1.89-4.21,4.21-4.21c1.703,0,3.178,1.023,3.841,2.494l4.717-4.717	c-1.961-2.602-5.065-4.277-8.559-4.277C9.81,16.58,5,21.38,5,27.29c0,3.491,1.691,6.59,4.288,8.547l4.71-4.71	C12.53,30.469,11.5,28.999,11.5,27.29z'
                  ></path>
                </svg>

                {/* Colored Google SVG */}
                <svg
                  className='colored-svg'
                  xmlns='http://www.w3.org/2000/svg'
                  x='0px'
                  y='0px'
                  width='48'
                  height='48'
                  viewBox='0 0 48 48'
                >
                  <path
                    fill='#1976d2'
                    d='M38.193,18.359c-0.771-2.753-2.319-5.177-4.397-7.03l-4.598,4.598	c1.677,1.365,2.808,3.374,3.014,5.648v1.508c0.026,0,0.05-0.008,0.076-0.008c2.322,0,4.212,1.89,4.212,4.212S34.61,31.5,32.288,31.5	c-0.026,0-0.05-0.007-0.076-0.008V31.5h-6.666H24V38h8.212v-0.004c0.026,0,0.05,0.004,0.076,0.004C38.195,38,43,33.194,43,27.288	C43,23.563,41.086,20.279,38.193,18.359z'
                  ></path>
                  <path
                    fill='#ffe082'
                    d='M19.56,25.59l4.72-4.72c-0.004-0.005-0.008-0.009-0.011-0.013l-4.717,4.717	C19.554,25.579,19.557,25.584,19.56,25.59z'
                    opacity='.5'
                  ></path>
                  <path
                    fill='#90caf9'
                    d='M19.56,25.59l4.72-4.72c-0.004-0.005-0.008-0.009-0.011-0.013l-4.717,4.717	C19.554,25.579,19.557,25.584,19.56,25.59z'
                    opacity='.5'
                  ></path>
                  <path
                    fill='#ff3d00'
                    d='M24,7.576c-8.133,0-14.75,6.617-14.75,14.75c0,0.233,0.024,0.46,0.035,0.69h6.5	c-0.019-0.228-0.035-0.457-0.035-0.69c0-4.549,3.701-8.25,8.25-8.25c1.969,0,3.778,0.696,5.198,1.851l4.598-4.598	C31.188,9.003,27.761,7.576,24,7.576z'
                  ></path>
                  <path
                    fill='#90caf9'
                    d='M15.712,31.5L15.712,31.5c-0.001,0-0.001,0-0.002,0c-0.611,0-1.188-0.137-1.712-0.373	l-4.71,4.71C11.081,37.188,13.301,38,15.71,38c0.001,0,0.001,0,0.002,0v0H24v-6.5H15.712z'
                    opacity='.5'
                  ></path>
                  <path
                    fill='#4caf50'
                    d='M15.712,31.5L15.712,31.5c-0.001,0-0.001,0-0.002,0c-0.611,0-1.188-0.137-1.712-0.373l-4.71,4.71	C11.081,37.188,13.301,38,15.71,38c0.001,0,0.001,0,0.002,0v0H24v-6.5H15.712z'
                  ></path>
                  <path
                    fill='#ffc107'
                    d='M11.5,27.29c0-2.32,1.89-4.21,4.21-4.21c1.703,0,3.178,1.023,3.841,2.494l4.717-4.717	c-1.961-2.602-5.065-4.277-8.559-4.277C9.81,16.58,5,21.38,5,27.29c0,3.491,1.691,6.59,4.288,8.547l4.71-4.71	C12.53,30.469,11.5,28.999,11.5,27.29z'
                  ></path>
                </svg>
              </>
            ) : (
              <>
                {/* White Azure SVG */}
                <svg
                  className='white-svg'
                  xmlns='http://www.w3.org/2000/svg'
                  x='0px'
                  y='0px'
                  width='48'
                  height='48'
                  viewBox='0 0 48 48'
                >
                  <path
                    fill='#ffffff'
                    d='M46 40L29.317 10.852 22.808 23.96 34.267 37.24 13 39.655zM13.092 18.182L2 36.896 11.442 35.947 28.033 5.678z'
                  ></path>
                </svg>

                {/* Colored Azure SVG */}
                <svg
                  className='colored-svg'
                  xmlns='http://www.w3.org/2000/svg'
                  x='0px'
                  y='0px'
                  width='48'
                  height='48'
                  viewBox='0 0 48 48'
                >
                  <path
                    fill='#035bda'
                    d='M46 40L29.317 10.852 22.808 23.96 34.267 37.24 13 39.655zM13.092 18.182L2 36.896 11.442 35.947 28.033 5.678z'
                  ></path>
                </svg>
              </>
            )}
            <span className='text-white text-xl font-medium logo-text'>
              {logo.name}
            </span>
          </div>
        ))}
      </div>
    </div>
  );
};

export default LogoBeltLeft;
